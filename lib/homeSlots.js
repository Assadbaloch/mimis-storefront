...
export async function getHomeSlots() {
  ...
  // `all` drives full-CMS mode: when the home page has its own sections they
  // compose the whole page in position order, and the hardcoded design steps
  // aside. Unpublishing the page brings the built-in design straight back,
  // which is the owner's one-click undo.
  return { ...slots, all: sections };
}
