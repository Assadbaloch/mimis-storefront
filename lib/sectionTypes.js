...
export const SECTION_TYPES = [
  ...
  },
  {
    type: 'hero_video',
    label: 'Video hero',
    description: 'Full-screen video banner with a large headline, two buttons and a link. The video stays vivid on the right while the text sits on a soft panel on the left.',
    icon: '🎥',
    fields: [
      { key: 'video', label: 'Background video', type: 'media', required: true },
      { key: 'headline', label: 'Headline (one line per line break)', type: 'textarea', required: true },
      { key: 'accent_word', label: 'Word to highlight in gold', type: 'text', placeholder: 'Yummy' },
      { key: 'height', label: 'Height', type: 'select', options: ['tall', 'full screen'], default: 'full screen' },
      { key: 'primary_label', label: 'Main button text', type: 'text', default: 'ORDER ONLINE →' },
      { key: 'primary_url', label: 'Main button link', type: 'text', default: '/menu' },
      { key: 'secondary_label', label: 'Second button text', type: 'text', default: 'VIEW MENU' },
      { key: 'secondary_url', label: 'Second button link', type: 'text', default: '/menu' },
      { key: 'link_label', label: 'Small link text', type: 'text', default: 'Join Rewards →' },
      { key: 'link_url', label: 'Small link', type: 'text', default: '/rewards' },
    ],
  },
  {
    type: 'text',
  ...
  },
  {
    type: 'trust_bar',
    label: 'Trust bar',
    description: 'A row of short proof points — sourcing, freshness, locations, ordering. Renders as a full-width band of colour.',
    icon: '🛡️',
    fields: [
      { key: 'background', label: 'Background', type: 'select', options: ['none', 'blue', 'red', 'yellow', 'dark'], default: 'red' },
      { key: 'columns', label: 'Columns', type: 'select', options: ['2', '3', '4'], default: '4' },
      {
        key: 'items',
        label: 'Proof points',
        type: 'repeater',
        add_label: 'Add proof point',
        item_title: 'title',
        item_fields: [
          { key: 'label', label: 'Small label above', type: 'text', placeholder: 'SOURCING' },
          { key: 'title', label: 'Headline', type: 'text', placeholder: '100% Zabiha Halal' },
          { key: 'media', label: 'Icon or picture (optional)', type: 'media' },
        ],
        default: [
          { label: 'SOURCING', title: '100% Zabiha Halal' },
          { label: 'INGREDIENTS', title: 'Fresh, Daily' },
          { label: 'FOOTPRINT', title: 'Two MI Locations' },
          { label: 'ORDERING', title: 'Pickup, Delivery & In Store' },
        ],
      },
    ],
  },
  {
    ...
    description: 'Quotes from real customers, each with a star rating and an optional photo.',
    icon: '💬',
    fields: [
      { key: 'eyebrow', label: 'Small label above headline', type: 'text', default: 'What People Say' },
      { key: 'headline', label: 'Headline', type: 'text', default: 'Loved across Madison Heights & Warren' },
      { key: 'subtext', label: 'Sub-text', type: 'textarea' },
      { key: 'background', label: 'Background', type: 'select', options: ['none', 'blue', 'red', 'yellow', 'dark'], default: 'none' },
      { key: 'columns', label: 'Columns', type: 'select', options: ['2', '3', '4'], default: '3' },
      // Source is declared here ahead of the Google integration so the block is
      // a drop-in when the API key lands -- switching a select rather than
      // rebuilding the section and re-entering its copy.
      { key: 'source', label: 'Where reviews come from', type: 'select', options: ['typed in below', 'live from Google'], default: 'typed in below' },
      { key: 'limit', label: 'Max reviews to show', type: 'number', default: 3 },
      {
        key: 'items',
        label: 'Reviews',
        type: 'repeater',
        ...
        item_title: 'author',
        item_fields: [
          { key: 'quote', label: 'What they said', type: 'textarea', required: true },
          { key: 'author', label: 'Name', type: 'text' },
          { key: 'location', label: 'Location / source', type: 'text', placeholder: 'Madison Heights' },
          { key: 'rating', label: 'Stars', type: 'select', options: ['5', '4', '3', '2', '1'], default: '5' },
          { key: 'media', label: 'Photo (optional)', type: 'media' },
        ],
        default: [],
      },
    ],
...
export function defaultConfigFor(type) {
  ...
  for (const f of def.fields) {
    ...
    // Deep-copy container defaults. A repeater's default is an array of objects;
    // assigning it by reference would make every section created from this type
    // share one array, so editing one trust bar would silently edit them all.
    config[f.key] = (f.default && typeof f.default === 'object')
